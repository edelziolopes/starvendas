...
class Produtos
{
  ...
  {
      $conn = new Database();
      // A linha 'p.id_categoria' foi adicionada à query. Este era o erro.
      $result = $conn->executeQuery('SELECT
          ...
          p.descricao,
          c.nome AS categoria,
          p.id_categoria      -- ESTA LINHA ESTAVA FALTANDO
      FROM tb_produtos p
      JOIN tb_categorias c ON p.id_categoria = c.id
      ORDER BY p.id DESC'); // Adicionar um ORDER BY é uma boa prática
